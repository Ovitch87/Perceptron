...
# Train a perceptron to separate Versicolor from the others
# Train-test split
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=123)

# Update target labels to differentiate Versicolor (class 1) from the others
y_train_versicolor = np.where(y_train == 1, 1, -1)
y_test_versicolor = np.where(y_test == 1, 1, -1)

# Instantiate and train the perceptron
perceptron = Perceptron(iterations=500, activation_function=sgn_function, random_seed=42)
perceptron.fit(X_train[:, :3], y_train_versicolor)

# Make predictions on the test set
predictions = perceptron.predict(X_test[:, :3])
accuracy = accuracy(y_test_versicolor, predictions)
...
plt.title('3D Plot of Iris Dataset with Versicolor Separation Plane')
plt.show()
...
plt.title("Perceptron Training Versicolor - Number of Errors over Iterations")
plt.show()